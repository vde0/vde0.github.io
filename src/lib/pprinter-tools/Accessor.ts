...
export class Accessor<F extends string = string> implements IAccessor<F> {
	...
	get on(): IListenerChest<AccessorEventMap>['on'] {
		return privateContext.get(this, 'chest')!.on;
	}
	get off(): IListenerChest<AccessorEventMap>['off'] {
		return privateContext.get(this, 'chest')!.off;
	}
	get once(): IListenerChest<AccessorEventMap>['once'] {
		return privateContext.get(this, 'chest')!.once;
	}
	get offAll(): IListenerChest<AccessorEventMap>['offAll'] {
		return privateContext.get(this, 'chest')!.offAll;
	}
	get exec(): IListenerChest<AccessorEventMap>['exec'] {
		return privateContext.get(this, 'chest')!.exec;
	}
	...
	getState(): boolean {
		return privateContext.get(this, 'state')!;
	}
}
