...
export const useChatFeed = (): MsgItem[] => {
	...
	useEffect(() => {
		...
		const update: Listener<ChatHistoryEventMap['add' | 'delete' | 'clear']> = () =>
			setFeed(getFeed());
		...
		return () =>
			unlisten<ChatHistory, ChatHistoryEventMap>(history, {
				[CHAT_HISTORY_EVENTS.ADD]: update,
				[CHAT_HISTORY_EVENTS.DELETE]: update,
				[CHAT_HISTORY_EVENTS.CLEAR]: update,
			});
...
export const useLocalChatter = (): [SymbolChatter, Send, ContentMedia] => {
	const chatUnit = useChatUnit();
	const [localMedia, setLocalMedia] = useState<ContentMedia>(
		chatUnit.getMedia(chatUnit.localChatter) ?? null
	);

	const send = useCallback<Send>(
		(msgText) => chatUnit.history.add(msgText, chatUnit.localChatter),
		[chatUnit]
	);
...
export const useRemoteChatter = (): [SymbolChatter, Send, ContentMedia] => {
	const chatUnit = useChatUnit();
	const [remoteMedia, setRemoteMedia] = useState<ContentMedia>(
		chatUnit.getMedia(chatUnit.remoteChatter) ?? null
	);

	const send = useCallback<Send>(
		(msgText) => chatUnit.history.add(msgText, chatUnit.remoteChatter),
		[chatUnit]
	);
