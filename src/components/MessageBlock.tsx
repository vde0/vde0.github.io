...
const MessageBlock: React.FC<MessageBlockProps> = ({ sender, text, direction }) => (
	<section
		className={`mb-2 p-2 rounded-2xl w-2/3 relative
        ${
					direction === 'left'
						? 'bg-gray rounded-bl-none left-2'
						: 'bg-light-blue rounded-br-none right-2 ml-auto'
				}`}
	>
		<span
			className={`block font-semibold ${direction === 'left' ? 'text-light-blue' : 'text-orange'}`}
		>
			{sender}
		</span>
